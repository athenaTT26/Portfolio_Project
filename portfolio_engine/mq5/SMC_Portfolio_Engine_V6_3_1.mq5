//+------------------------------------------------------------------+
//|  SMC_Portfolio_Engine_V6_3_1.mq5                                          |
...
// ATHENA candidate component scores from the latest ACI evaluation
double g_AthenaHTFScore          = 0.0;
double g_AthenaLiquidityScore    = 0.0;
double g_AthenaFVGScore          = 0.0;
double g_AthenaDisplacementScore = 0.0;
double g_AthenaVolumeScore       = 0.0;
double g_AthenaVolatilityScore   = 0.0;
double g_AthenaSessionScore      = 0.0;
...
string Athena_CurrentSessionLabel()
{
   MqlDateTime t;
   TimeToStruct(TimeCurrent(), t);
   if(t.hour >= 7 && t.hour < 12)  return "London_AM";
   if(t.hour >= 12 && t.hour < 15) return "London_NY_Transition";
   if(t.hour >= 15 && t.hour < 19) return "London_NY_Overlap";
   if(t.hour >= 19 && t.hour < 22) return "NewYork_PM";
   return "Outside_Prime";
}

bool Athena_DecisionAccepted(const string decision)
{
   return (decision == "ENTER" || decision == "ACCEPT" || decision == "APPROVED");
}
                      ...
                      string &detail)
{
   ...
   g_AthenaHTFScore          = 0.0;
   g_AthenaLiquidityScore    = 0.0;
   g_AthenaFVGScore          = 0.0;
   g_AthenaDisplacementScore = 0.0;
   g_AthenaVolumeScore       = 0.0;
   g_AthenaVolatilityScore   = 0.0;
   g_AthenaSessionScore      = 0.0;

   if(h4OK && DirectionalAlignmentOK(direction))
   {
      score += InpAciWeightHTF;
      g_AthenaHTFScore = InpAciWeightHTF;
   ...
   if(sweepOK)
   {
      ...
      g_AthenaLiquidityScore = InpAciWeightLiquidity;
   ...
   if(bosOK)
   {
      ...
      g_AthenaDisplacementScore = InpAciWeightDisplacement;
   ...
   if(fvgOK)
   {
      ...
      g_AthenaFVGScore = InpAciWeightFVG;
   ...
   score += volPts;
   g_AthenaVolatilityScore = volPts;
   ...
   score += sessionPts;
   g_AthenaSessionScore = sessionPts;
...
void ACI_LogCandidate(string dir,
                      ...
                      string detail)
{
   // Preserve existing v6.2 candidate CSV output for backward compatibility.
   if(InpAciLogCandidates)
   {
   ...
   // ATHENA v6.3.1: emit structured candidate event through EventBus.
   AthenaCandidateEvent event;
   event.symbol               = _Symbol;
   event.direction            = dir;
   event.candidate_time       = TimeCurrent();
   event.accepted             = Athena_DecisionAccepted(decision);
   event.rejection_reason     = event.accepted ? "" : decision;
   event.regime               = RegimeGate_Label();
   event.volatility_state     = VolGate_Label();
   event.session_name         = Athena_CurrentSessionLabel();
   event.spread_points        = (SymbolInfoDouble(_Symbol, SYMBOL_ASK) - SymbolInfoDouble(_Symbol, SYMBOL_BID)) / _Point;
   event.atr_value            = LM_GetATR(_Symbol, InpEntryTF, 14);
   event.htf_score            = g_AthenaHTFScore;
   event.liquidity_score      = g_AthenaLiquidityScore;
   event.fvg_score            = g_AthenaFVGScore;
   event.displacement_score   = g_AthenaDisplacementScore;
   event.volume_score         = g_AthenaVolumeScore;
   event.volatility_score     = g_AthenaVolatilityScore;
   event.session_score        = g_AthenaSessionScore;
   event.total_score          = aciScore;
   event.market_quality_score = aciScore;

   AthenaBus.EmitCandidate(event);
}
...
int OnInit()
{
   ...
   AthenaBus.Init("SMC_Portfolio_Engine_v6.3.1", EnableAthena, AthenaDebug, "ATHENA");
   string v6Reason = "";
   ...
   PrintFormat("SMC_Portfolio_Engine_v6.3.1 | Symbol=%s | Magic=%d | EqPips=%.1f | SLBuf=%.1f | VolGate=%s",
